...
- **act/Colima preflight**: `make act-smoke` invokes only the read-only `scripts/act-smoke.sh` capability probe. Missing `act`, a stopped Colima VM, or an unavailable Docker API returns `SKIPPED_UNAVAILABLE` with exit 0; it never invokes a workflow, uploads SARIF, or runs release/security steps. Secrets, privileged mode, host/socket mounts, and SSH/cloud credentials remain disallowed.
- **act/Colima advisory smoke**: `make act-smoke-run` validates `tests/fixtures/act/pull-request.json` and the exact `route-observe` job in `ci-route-observe.yml`, then consumes the Colima CLI/Docker API preflight. Missing runtime returns `SKIPPED_UNAVAILABLE` and records no synthetic benchmark; available-runtime execution remains advisory, defaults to `network=none`, and requires the explicit `ACT_SMOKE_NETWORK=host` exception for the route's read-only `git fetch`. See `docs/reports/act-smoke-baseline.md`.
