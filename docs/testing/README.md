...
- **TDD promotion evidence**: `docs/testing/tdd-evidence.json` is the durable AC-to-test map for closed E2E-COV-010.1. It requires RED/GREEN/REFACTOR commands and explicit `passed`, `failed`, `skipped`, or `inconclusive` promotion status at unit, integration, functional, E2E, and agentic levels. The E2E-COV-010.2 synthetic-transition trace harness is offline-tested, records explicit safety transitions, and writes an independent 0600 JSONL audit sink, but does not invoke a fake tool; agentic promotion remains `skipped` until that replay evidence is reviewed.
