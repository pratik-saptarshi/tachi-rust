...
- [x] Hosted timing verification independently binds artifact provenance to GitHub run metadata, and local log retention/redaction policy is contract-tested (`E2E-COV.2`). Workflow/status/run/event/attempt including workflow_dispatch, exact push/ref semantics, explicit PR merge-commit requirement, synthetic PR source/ref rejection, runner provenance, nullglob artifact matching, fail-closed cleanup, exact tiny-log caps, path normalization, expanded redaction, offline unit/aggregate/receipt schema validation, and schema-backed 0600 ephemeral-cleanup receipts are tested and Beads-closed.
