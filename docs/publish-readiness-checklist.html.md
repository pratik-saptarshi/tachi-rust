...
- [ ] Hosted timing verification independently binds artifact provenance to GitHub run metadata, and local log retention/redaction policy is contract-tested (`E2E-COV.2`). Workflow/status/run/event/attempt, exact push-ref, runner provenance, bounded-log, path-normalization, expanded-redaction, and schema-backed 0600 ephemeral-cleanup receipt contracts are now tested; remaining work is complete schema keyword enforcement and valid new-format PR fixture coverage.
