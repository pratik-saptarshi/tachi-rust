...
- [x] Docker fallback only — not publish evidence: the act wrapper is opt-in and advisory. `make act-smoke` and `make act-smoke-run` enforce trusted fixture/workflow paths, explicit runtime selection and endpoint binding, empty secrets, no daemon-socket mount, no privileged mode, `--rm`, default `network=none`, explicit recorded host-network exception only for the synthetic route fetch, image digest/resource provenance, side-effect fields, cleanup comparison, timeout, and `SKIPPED_UNAVAILABLE` behavior. Docker/Colima fallback evidence passed in two cold runs (13,710/13,727 ms; 646/542 ms image pulls) and six warm runs (13,024–13,470 ms; median 13,258.5 ms) with no hosted artifact/SARIF/release/security side effects.
