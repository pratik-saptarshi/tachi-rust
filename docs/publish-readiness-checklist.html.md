...
- [x] Hosted timing verification independently binds artifact provenance to GitHub run metadata, and local log retention/redaction policy is contract-tested (`E2E-COV.2`). Workflow/status/run/event/attempt, exact push-ref, synthetic PR source/ref rejection, runner provenance, bounded-log, path-normalization, expanded-redaction, offline unit/aggregate/receipt schema validation, and schema-backed 0600 ephemeral-cleanup receipt contracts are tested and Beads-closed.
