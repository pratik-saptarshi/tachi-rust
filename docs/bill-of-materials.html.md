...
| Coverage gate | `make llvm-cov`; governed nightly branch command with explicit `RUSTC`/`RUSTDOC`/`LLVM_COV`/`LLVM_PROFDATA` paths | Latest current stable coverage reports 92.60% regions and 93.24% lines. The older 90.22% / 90.56% values belong to the historical durable publish-gate snapshot. Nightly 1.99.0 records 85.15625% branch coverage (1,408 total / 210 missed), meeting the requested 85% target; the latest isolated-target invocation exited 0 but did not emit a newly captured numeric summary. |
...
| act/Colima capability preflight | `E2E-COV-009.1` | Complete advisory preflight: `make act-smoke` / `scripts/act-smoke.sh` use the Colima CLI and Docker API to report runtime/version/provider, local endpoint identity, image digest, architecture, CPU/memory profile, policy, and side-effect fields; missing or stopped Colima returns `SKIPPED_UNAVAILABLE` without workflow invocation. |
| act/Colima advisory smoke | `E2E-COV-009.2` | Complete advisory Colima smoke: `make act-smoke-run` validates the exact trusted route-observe workflow/job and synthetic fixture, uses empty/sanitized secrets/no daemon socket/`--rm`, skips hosted artifact upload under `ACT_SMOKE=true`, verifies the pinned image digest and trusted workflow hash, and validates `route.json` in-container. Two cold runs (32,384/32,864 ms; 556/562 ms pulls) and five warm runs (29,753–31,347 ms; median 31,225 ms) passed with verified cleanup and no hosted side effects. See `docs/reports/act-smoke-baseline.md`. |
...
| Advisory workflow emulation | `scripts/act-smoke.sh`, `scripts/act-smoke-run.sh`, `tests/fixtures/act/pull-request.json`, `make act-smoke-run` | Complete advisory Colima capability. Opt-in only; Colima CLI/API provenance is recorded, with no secrets, daemon-socket mount, privileged mode, hosted artifact upload, release/security side effects, or hosted-CI claims. Structured setup-failure evidence and serial cold/warm measurements are covered. |
