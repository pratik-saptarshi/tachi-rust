...
| Coverage gate | `make llvm-cov`; governed nightly branch command with explicit `RUSTC`/`RUSTDOC`/`LLVM_COV`/`LLVM_PROFDATA` paths | Standalone stable coverage passes at 90.22% regions and 90.56% lines. Nightly 1.99.0 records 85.15625% branch coverage (1,408 total / 210 missed), meeting the requested 85% target. The complete durable `make publish-gate` run exited 0 after security, supply-chain, runner, coverage, release, and contract stages. |
...
| Publish gate | `make publish-gate` | Complete historical durable run on `feat/e2e-publish-gate-closeout-007` exited 0. It produced local-full runner `20260712T195832Z-73032` with 8/8 units, 0 failures/timeouts/cancellations, 114 active modules (13 unit, 96 integration, 1 smoke, 4 E2E), 85.15625% nightly branch coverage, stable 90.56% lines / 90.22% regions, and passed the security, supply-chain, docs, release, and workflow contract stages. The current audit refresh is 119 modules (13 unit, 101 integration, 1 smoke, 4 E2E); this refresh is synchronized separately from the historical gate snapshot. |
| Local CI-parity runner | `.github/ci-test-units.json`, `schemas/ci-test-units.schema.json`, `schemas/ci-run-result.schema.json`, `schemas/ci-run-aggregate.schema.json`, `schemas/ci-cleanup-receipt.schema.json`, `scripts/ci-local-runner.sh`, `make test`, `make test-route` | E2E-COV-007.3 evidence complete. Terminal local-full run `20260712T173705Z-72397` passed 8/8 in 536,162 ms (compile-and-test 466,327 ms; test-slice 68,906 ms); labeled warm route-equivalent evidence passed 8/8 in 304,650 ms; controlled cold route-equivalent evidence passed 8/8 in 321,636 ms. JSON provenance records stage totals, cache context, toolchain, cleanup, and outcome; E2E-COV-008 hosted timing/reliability evidence is now complete with documented sample limitations. |
...
| Timing provenance/privacy hardening | `E2E-COV.2` | Complete TDD slice: hosted metadata, push/workflow_dispatch ref binding, explicit-commit PR verification with valid/rejected synthetic source/ref provenance, fail-closed rejection of unsupported rerun-attempt artifact selection, local ephemeral/retain policy, offline unit/aggregate/receipt schema keyword validation, fail-closed cleanup, exact bounded logs, path-normalized metadata, expanded credential redaction, and 0600 `ci-cleanup-receipt.schema.json` evidence are implemented and Beads-closed. |
| act/Podman capability preflight | `E2E-COV-009.1` | Partial TDD closeout: `make act-smoke` / `scripts/act-smoke.sh` now report selected runtime/API compatibility, image digest, architecture, CPU/memory profile, policy, and side-effect fields; missing or stopped runtimes return `SKIPPED_UNAVAILABLE` without workflow invocation. Docker/Colima fallback is explicit and measured; stable rootless Podman machine/API evidence remains open. |
| act/Podman advisory smoke | `E2E-COV-009.2` | Partial TDD closeout: `make act-smoke-run` validates the exact trusted route-observe workflow/job and synthetic fixture, uses empty/sanitized secrets/no daemon socket/`--rm`, skips hosted artifact upload under `ACT_SMOKE=true`, verifies the pinned image digest and trusted workflow hash, and validates `route.json` in-container. Docker/Colima two cold runs (13,710/13,727 ms; 646/542 ms pulls), six warm runs (13,024–13,470 ms; median 13,258.5 ms), and cached resource samples (27,636 ms, 13,249 ms, 14,309 ms, 13,732 ms, and 13,940 ms; 2 CPUs; 4,104,118,272-byte memory profile; 571,284,183-byte image) passed with derived cache state and verified cleanup/log/artifact handling; Podman measurements remain open. See `docs/reports/act-smoke-baseline.md`. |
| TDD evidence promotion | `E2E-COV-010.1` | Complete contract slice: `docs/testing/tdd-evidence.json` and `crates/tachi-core/tests/tdd_evidence_contract.rs` map acceptance criteria to RED/GREEN/REFACTOR evidence and explicit promotion status across unit, integration, functional, E2E, and agentic levels. Agentic replay is now promoted `passed` through E2E-COV-010.2. |
| Deterministic agentic replay | `E2E-COV-010.2` | Complete child: `scripts/agentic-replay.sh`, `tests/fixtures/agentic/fake-tool.sh`, `tests/fixtures/agentic/replay.json`, and `crates/tachi-core/tests/agentic_replay_contract.rs` enforce offline fixture shape, fixed fake-tool invocation, bounded timeout/cancel/circuit behavior, denial non-invocation, descendant cleanup, deterministic repeated bytes, exact independent 0600 JSONL audit correlation, output/audit collision rejection, and network denial. Final code review passed; promotion is `passed`. |
| Advisory workflow emulation | `scripts/act-smoke.sh`, `scripts/act-smoke-run.sh`, `tests/fixtures/act/pull-request.json`, `make act-smoke-run` | Partial E2E-COV-009 capability. Opt-in only; Podman remains the preferred measured runtime, Docker/Colima is an explicitly labeled fallback; no secrets, daemon-socket mount, privileged mode, hosted artifact upload, release/security side effects, or hosted-CI claims. The runner now emits bounded structured failure evidence for runtime setup/probe failures; rootless Podman cold/warm proof remains open. |
...
| CI gate | GitHub Actions run status | Release, security, lint, workspace, init, and dynamic CodeQL workflows for post-merge `2bf68bb` are green; the follow-up documentation merge `f34f5a5` is also green on all 20 PR checks. Docs-specific validation remains represented by the local/version gates and PR checks, not claimed as a main-push workflow. Runs are recorded in the readiness checklist and integration log. |
