...
| act/Podman advisory smoke | `E2E-COV-009.2` | Partial TDD closeout: `make act-smoke-run` validates the trusted synthetic fixture/workflow, runs only `route-observe`, uses empty secrets/no daemon socket/`--rm`, skips hosted artifact upload under `ACT_SMOKE=true`, and validates `route.json` in-container. Docker/Colima two cold runs (13,710/13,727 ms; 646/542 ms pulls) and six warm runs (13,024–13,470 ms; median 13,258.5 ms) passed with verified cleanup; Podman measurements remain open. See `docs/reports/act-smoke-baseline.md`. |
