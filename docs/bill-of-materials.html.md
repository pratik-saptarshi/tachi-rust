...
| act/Podman capability preflight | `E2E-COV-009.1` | Partial TDD closeout: `make act-smoke` / `scripts/act-smoke.sh` report runtime, architecture, policy, resource, and side-effect fields; missing act/Podman returns `SKIPPED_UNAVAILABLE` without workflow invocation. Available-runtime Docker-API, digest, and resource evidence remains open; advisory lane is not a publish or hosted-CI gate. |
| act/Podman advisory smoke | `E2E-COV-009.2` | Partial TDD closeout: `make act-smoke-run` validates `tests/fixtures/act/pull-request.json` and the `route-observe` job definition in `ci-route-observe.yml`, consumes preflight, and records an unavailable-safe result with no workflow/SARIF/release/security side effects. See `docs/reports/act-smoke-baseline.md`; available-runtime cold/warm resource measurements remain open. |
