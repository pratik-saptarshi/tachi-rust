...
| Timing provenance/privacy hardening | `E2E-COV.2` | Partial TDD closeout: `ci_timing_artifacts_contract.rs` now fails closed on wrong workflow/run status and binds artifact runner event/attempt/run ID; the local runner has explicit `CI_LOCAL_RETENTION=ephemeral|retain` with ephemeral cleanup by default. Remaining work is stronger push/PR commit provenance, bounded/path-safe logs, expanded redaction, and aggregate schema enforcement; the issue remains open. |
