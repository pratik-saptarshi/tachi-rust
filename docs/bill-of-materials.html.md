...
| Publish gate | `make publish-gate` | Fresh host-assisted run completed through advisory, security, coverage, runner, release, and cleanup stages. Terminal local-full cleanup receipt is `20260713T045400Z-79992`; advisory database fetched 1,160 records and cargo deny reported `advisories ok, bans ok, licenses ok, sources ok`. Current audit is 119 modules (13 unit, 101 integration, 1 smoke, 4 E2E); current stable coverage is 93.24% lines / 92.60% regions. Historical closeout runner `20260712T195832Z-73032` and its 114/96, 90.56% / 90.22% snapshot remain separately labeled. |
...
| act/Colima advisory smoke | `E2E-COV-009.2` | Complete advisory Colima smoke: `make act-smoke-run` validates the exact trusted route-observe workflow/job and synthetic fixture, uses empty/sanitized secrets/no daemon socket/`--rm`, skips hosted artifact upload under `ACT_SMOKE=true`, verifies the pinned image digest and trusted workflow hash, and validates `route.json` in-container. Existing two cold/five warm baseline samples passed; live revalidation on 2026-07-13 passed in 27,260 ms with `ACT_SMOKE_RUNTIME=colima ACT_SMOKE_NETWORK=host`, verified cleanup, and no hosted side effects. The secure `network=none` default remains unavailable for this checkout-dependent synthetic route. See `docs/reports/act-smoke-baseline.md`. |
...
| CI gate | GitHub Actions run status | Historical post-merge `2bf68bb` and documentation merge `f34f5a5` evidence remains labeled; the current PR #33 feature head has 20 terminal checks passed and 0 failures, including CodeQL, supply-chain, gitleaks, clippy, rustfmt, route, and workspace suites. Copilot review is optional under the permanently updated secure ruleset, so its quota exhaustion is non-blocking; remaining protection is review-thread resolution. Auto-merge remains enabled and no administrator bypass is used. |
