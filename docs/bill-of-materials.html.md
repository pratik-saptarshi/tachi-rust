...
| Deterministic agentic replay | `E2E-COV-010.2` | Partial harness: `scripts/agentic-replay.sh`, `tests/fixtures/agentic/replay.json`, and `crates/tachi-core/tests/agentic_replay_contract.rs` enforce offline fixture shape, governed allowlist, exact safety cases, deterministic audit correlation, and network denial. Promotion remains skipped until behavioral timeout/cancel/circuit execution and an independent audit sink are proven. |
