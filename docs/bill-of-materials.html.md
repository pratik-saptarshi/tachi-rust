...
| Local/hosted performance and reliability evidence | Local runner `results.json`, workflow job summaries, `make rt-ci-latency-evidence` | Required before publish closeout: compare per-stage and overall wall time, queue/run timing where available, pass/fail/timeout/cancellation rates, artifact integrity, cleanup, and reproducibility across cold/warm runs without weakening gates. |
