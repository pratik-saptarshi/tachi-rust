...
| Timing provenance/privacy hardening | `E2E-COV.2` | Complete TDD slice: hosted metadata, push/workflow_dispatch ref binding, explicit-commit PR verification with valid/rejected synthetic source/ref provenance, local ephemeral/retain policy, offline unit/aggregate/receipt schema keyword validation, fail-closed cleanup, exact bounded logs, path-normalized metadata, expanded credential redaction, and 0600 `ci-cleanup-receipt.schema.json` evidence are implemented and Beads-closed. |
...
| Deterministic agentic replay | `E2E-COV-010.2` | Partial synthetic-transition trace harness: `scripts/agentic-replay.sh`, `tests/fixtures/agentic/replay.json`, and `crates/tachi-core/tests/agentic_replay_contract.rs` enforce offline fixture shape, governed allowlist, explicit approval/denial/timeout/cancel/circuit traces, deterministic repeated bytes, exact independent 0600 JSONL audit correlation, output/audit collision rejection, and network denial. The harness does not invoke a fake tool; promotion remains skipped pending that execution review. |
