...
| Timing provenance/privacy hardening | `E2E-COV.2` | Complete TDD slice: hosted metadata and exact push-ref binding, valid/rejected synthetic PR source/ref provenance, local ephemeral/retain policy, offline unit/aggregate/receipt schema keyword validation, bounded logs, path-normalized metadata, expanded credential redaction, and 0600 `ci-cleanup-receipt.schema.json` evidence are implemented and Beads-closed. |
