...
| CI gate | GitHub Actions run status | Historical post-merge `2bf68bb` and documentation merge `f34f5a5` evidence remains labeled; current PR #33 feature commit `9c8a4f7` has 20 terminal checks passed and 0 failures, including CodeQL, supply-chain, gitleaks, clippy, rustfmt, route, and workspace suites. PR #33 remains OPEN/BLOCKED under the active secure ruleset because the Copilot review attempt hit quota exhaustion; auto-merge remains enabled and no administrator bypass is used. |
