...
| Timing provenance/privacy hardening | `E2E-COV.2` | Partial TDD closeout: hosted metadata and exact push-ref binding, local ephemeral/retain policy, schema-backed required/property checks, bounded logs, path-normalized metadata, expanded credential redaction, and 0600 `ci-cleanup-receipt.schema.json` evidence are implemented. Remaining work is complete schema keyword validation and valid new-format PR fixture coverage; the issue remains open. |
