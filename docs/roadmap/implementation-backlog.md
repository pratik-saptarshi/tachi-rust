...
- Open: `RT-TC`, `RT-TC-003`, `RT-TC-006`
...
- Done: all remaining Beads issues exported in `../../.beads/issues.jsonl`,
  ...
  `DOC-*`, `RT-TC-001`, `RT-TC-002`, `RT-TC-004`, `RT-TC-005`, and completed `AQ-*` / `RT-*`
  migration slices.
...
- Priority `1`: `RT-TC-003` semantic CI tests remains open. `RT-TC-004`
  added pinned `cargo-hack`/`cargo-llvm-cov` manual/scheduled canaries, and
  `RT-TC-005` resolved `src-tauri` as an explicitly excluded standalone
  adapter with its own lockfile and manual/scheduled compatibility lane.
...
- `bd ready --json` currently returns the open `RT-TC` hierarchy except for
  closed `RT-TC-001`, `RT-TC-002`, `RT-TC-004`, and `RT-TC-005`.
