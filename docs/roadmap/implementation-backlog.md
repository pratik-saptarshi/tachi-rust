...
- Current baseline after `E2E-COV-003`: three E2E modules (`crates/tachi-cli/tests/e2e_artifacts.rs`, `crates/tachi-desktop/tests/e2e_command_journey.rs`, and `crates/tachi-shell/tests/init_substitution.rs`), 111 active modules, 85.25% lines / 84.77% regions, and no current branch-coverage report.
