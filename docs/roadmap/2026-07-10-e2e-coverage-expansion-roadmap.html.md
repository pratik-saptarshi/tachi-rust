...
**Status**: Remediation plan integrated from overseer review; E2E-COV-007.1 branch target met, aggregate local publish-gate closeout pending
**Date**: 2026-07-10
...
5. Verify with focused runner contract tests, each package suite independently, the full local runner, and remote `rust workspace tests` CI. Do not close `E2E-COV-007.3` until either the full local gate terminates green or the documented workaround is deterministic, contract-tested, and clearly marked as an environment limitation.
...
4. Add a scheduled maintenance check that reports the current upstream v4 release and opens a dependency/update task, while the publish gate rejects stale v3 references and undocumented historical references only where they appear in active workflow/config surfaces.
