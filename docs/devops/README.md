...
The **Substitution Surface Hardening** feature (PR #249, feature branch `248-substitution-surface-hardening` squash-merged on 2026-05-04, merge commit `6db9a25`) shipped the Rust init matrix workflow, a baseline-regeneration tooling script, and two test-only environment-variable contracts to lock down the `scripts/init.sh` placeholder-substitution surface against bash-version drift. The feature is **template-maintenance only** — adopters see zero runtime impact, but anyone extending the upstream template MUST understand the new contracts:

- **New CI workflow**: the Rust init matrix workflow now runs the Rust init matrix (`cargo test -q -p tachi-shell --test init_substitution`, `init_constitution`, `init_defaults_env`, `init_precommit_matrix`, `init_timing_trace`, `init_trace_summary`, plus the `active_docs_runtime_guidance` guard) on a 2-runner matrix (`macos-latest` + `ubuntu-latest`, `fail-fast: false`). The macOS leg is the strictest gate — bash 3.2.57 (Apple GPLv3 floor) — and the Ubuntu leg is the modern reference at bash 5.x. Both legs MUST pass for the workflow to close green. Trigger: `pull_request` only, path-filtered to the Rust init surface (per NFR-005). Full walkthrough in `docs/devops/CI_CD_GUIDE.md`.
- **Path filter discipline (NFR-005)**: the workflow does NOT fire on unrelated edits (docs, ADR text, agent-tier files). The trigger set is enumerated in the workflow and copied into `CI_CD_GUIDE.md`. Any future changes to the init surface MUST add the new file path to this filter; otherwise the gate silently bypasses the change.
...
- **Rust-only CI dependencies**: the workflow installs the stable Rust toolchain and does not need package installation at runtime. Cross-suite dependency notes remain only for archival fixtures and historical docs.
...
- **Performance characteristic on macOS**: the matrix's macOS leg is still the slowest runner, but the Rust suite removes the legacy test dependency and keeps the same bash 3.2.57 compatibility gate. Rust test timing remains bounded by the same shell hardening surfaces, and the historical 300s/360s timeout notes are retained only for archive context.
...
**Adopter action**: none. F-248 is template-maintenance only. Maintainers extending the substitution surface (new `{{...}}` placeholder, new shell helper under `.aod/scripts/bash/`) MUST: (a) add the new file path to the workflow's path filter, (b) extend the test suite if the new path introduces case logic in `template-substitute.sh`, and (c) regenerate the baseline tree via `tests/fixtures/regenerate-baseline.sh`. PRD: `docs/product/02_PRD/248-substitution-surface-hardening-2026-05-03.md`. Spec + tasks: `specs/248-substitution-surface-hardening/`. ADR: `docs/architecture/02_ADRs/ADR-038-placeholder-substitution-strategy.md`. Workflow walkthrough: `docs/devops/CI_CD_GUIDE.md` → "Rust Init Matrix Workflow (F-248)". Env-var contracts: `docs/devops/environment-variables.md`.

---

## Feature 277 Additions (2026-05-09)

The **Claude Permissions Baseline** feature (BLP-02 Wave 4 / F-4, PR #278, feature branch `277-claude-permissions-baseline` squash-merged on 2026-05-09 at `896588b`) shipped a curated `.claude/settings.json` permissions baseline (~110 LOC after Cat-1 dedup), `docs/standards/CLAUDE_PERMISSIONS.md` policy decision log (~289 LOC), and `ADR-041-claude-permissions-baseline.md` (~195 LOC with 6 alternatives-considered). This is a **Claude-Code-runtime governance** feature with **zero CI/CD or infrastructure surface area** — the baseline operates at agent-session start inside the Claude Code harness, not in CI pipelines or deployment environments:

- **No CI workflow changes**: F-4 touched zero `.github/workflows/*.yml` files. The `.claude/settings.json` file is read by the Claude Code harness at agent invocation time on the developer's local machine — it never executes inside `manifest-coverage`, `extract-coverage`, `stack-contract`, the Rust init matrix workflow, or `tachi-mmdc-preflight` workflows. CI continues to apply unchanged.
- **No new scripts, no script modifications, no Makefile changes**: F-4 touched zero `.sh` files, zero `Makefile` targets, zero `.aod/scripts/bash/` helpers. The baseline is pure declarative JSON enforced by Claude Code's permission model.
- **No env vars, no secrets, no Docker services, no deployment-environment changes**: F-4 ships no runtime, no daemon, no service, no networking surface. Staging and Production READMEs are intentionally unchanged.
- **New `.gitignore` exclusion (one adopter-facing change)**: `.claude/settings.local.json` is added to `.gitignore` per FR-003. The committed `.claude/settings.json` is the project-level baseline that adopters inherit via `git pull` (or `make update`); `settings.local.json` is reserved for per-developer / per-machine personal allows/denies that should never be committed. Adopters cloning the repo do NOT need to create `settings.local.json` — the baseline is fully functional standalone. The override file is only created if a developer chooses to layer personal preferences on top of the baseline.
- **AC-7 subdomain transitive-collapse anomaly (informational, not infra-blocking)**: During build-stage verification (Wave 11), an Option A disambiguation probe confirmed that the Claude Code harness transitively matches `WebFetch(domain:github.com)` against unlisted subdomains (e.g., `gist.github.com` auto-approved). This contradicts upstream Issues #15260, #11972, #1217 which documented non-transitive matching at filing time. The 19-domain network host-allowlist in `.claude/settings.json` still functions as defense-in-depth, but the 7 explicit `github.com`-family entries are subsumed by the parent `domain:github.com` rule via transitive collapse. Compaction is a tracked follow-up (T028 family per `specs/277-claude-permissions-baseline/tasks.md`), not a blocker. No DevOps action required — this is a Claude Code harness behavior observation, not a CI gate or deployment concern.
- **Two follow-up Issues filed at task-time** that touch DevOps surface (deferred — not part of F-4 itself):
  - **#280** (ICE 22 / I:5 C:7 E:8): pre-commit hook for `.claude/settings.json` jq-validity + AC-2 cross-check. **Local pre-commit hook proposal** — would add a developer-side guard against breaking the baseline JSON, NOT a CI gate.
  - **#281** (ICE 19 / I:6 C:6 E:7): CI integration for the F-4 verification recipe (jq-validity + AC-2 + AC-7 + AC-12 probes). AC-7 and AC-12 require an interactive Claude Code session (live permission-prompt observation), so a CI-runner version may degrade to documentation-only checks of jq-validity + AC-2 string-match assertions. AC-16 CI proposal was **deferred at /aod.spec time** — F-4 itself ships no CI integration.
- **Post-merge `/security` re-scan PASSED**: F-4's change set has zero SAST-eligible files (no `.py` / `.js` / `.ts` / `.sh`) and zero SCA-eligible manifests. The strict-protocol diff produced empty SAST + SCA paths; the security skill SKIPPED both axes and returned PASSED. SARIF artifact at `.security/reports/c99c46d0bab9.sarif` (empty `results[]` / `rules[]`); scan log appended to `.security/scan-log.jsonl` (chain_hash continuity verified).

**Adopter action**: none at the DevOps layer. Adopters who run `make update` will receive the new `.claude/settings.json` baseline + `CLAUDE_PERMISSIONS.md` decision log + ADR-041 via the standard F-129 manifest-driven sync. The baseline tightens default Claude Code permissions (network egress confined to a 19-domain allowlist, destructive operations gated behind `deny` and `ask` rules) but introduces zero CI gate, exit-code, or environment-variable change. Maintainers extending the baseline (adding a network domain, a destructive deny rule, a read-only permission) MUST: (a) update `.claude/settings.json`, (b) record the rationale in `docs/standards/CLAUDE_PERMISSIONS.md`, and (c) consider whether the change warrants an ADR amendment to ADR-041. PRD: `docs/product/02_PRD/277-claude-permissions-baseline-2026-05-08.md`. Spec + tasks: `specs/277-claude-permissions-baseline/`. ADR: `docs/architecture/02_ADRs/ADR-041-claude-permissions-baseline.md`. Policy log: `docs/standards/CLAUDE_PERMISSIONS.md`.

---

## Feature 282 Additions (2026-05-10)

The **Pre-commit Secret-Scanning Defaults** feature (BLP-02 Wave 4+ / F-5, PR #283, feature branch `282-pre-commit-secret-scanning-defaults` squash-merged on 2026-05-10 at `18378bd`) shipped a default-secure pre-commit gitleaks hook for new tachi adopters, an opt-in path for existing adopters, and a CI parity workflow as a back-stop for `--no-verify` deliberate bypass. F-5 closes BLP-02 (5/5) and the 5-feature enterprise-hardening initiative. This is a **local-tooling + CI** feature with **no staging/production deployment surface area** — the hook runs at developer-machine commit time and inside GitHub Actions on `pull_request`, never inside deployment pipelines:

- **New CI workflow**: `.github/workflows/gitleaks.yml` runs a full-repo gitleaks scan on every `pull_request` against `main`. Pinned to gitleaks **v8.30.1** with **SHA256 verification** (`551f6fc83ea457d62a0d98237cbad105af8d557003051f41f3e7ca7b3f2470eb`) of the upstream release tarball before extraction. Findings are uploaded to GitHub Code Scanning via `github/codeql-action/upload-sarif@v4` so PR reviewers see them inline. Runs on `ubuntu-latest`, `permissions: contents: read + security-events: write`, `continue-on-error: true` on the scan step so the SARIF upload always runs (`if: always()`). **Native gitleaks invocation** — does NOT use the proprietary `gitleaks-action@v2` because that action requires a paid `GITLEAKS_LICENSE` secret for org repos (per ADR-042 §Alternatives). Full walkthrough in `docs/devops/CI_CD_GUIDE.md` → "Gitleaks CI Parity Workflow (F-282)".
- **Full-repo scan, not PR-diff (Q5 ruling)**: PR-diff scanning would miss pre-existing un-scanned credentials in older commits. Full-repo catches them at merge time even if they predate the hook's introduction. Runtime budget: <30s for a tachi-sized tree. Trade-off: marginally higher CI cost vs. cleaner credential hygiene over the lifetime of the repo.
- **Rust init matrix path-filter delta**: the Rust init matrix gained Rust test module entries — `crates/tachi-shell/tests/init_substitution.rs`, `crates/tachi-shell/tests/init_constitution.rs`, `crates/tachi-shell/tests/init_defaults_env.rs`, `crates/tachi-shell/tests/init_precommit_matrix.rs`, `crates/tachi-shell/tests/init_timing_trace.rs`, `crates/tachi-shell/tests/init_trace_summary.rs`, and `crates/tachi-core/tests/active_docs_runtime_guidance.rs` — added to BOTH the trigger set and the `cargo test` invocation in lock-step. Edits to F-5's init.sh prompt block now correctly fire the Rust matrix on both `macos-latest` (bash 3.2.57 floor) and `ubuntu-latest` (bash 5.x reference).
- **New `.pre-commit-config.yaml` at repo root** (NEW): single-hook config invoking the gitleaks v8.30.1 binary via the wrapper script at `.aod/scripts/bash/precommit-wrap.sh`. The `rev` field starts as a tag (`v8.30.1`) for human readability; `pre-commit autoupdate --freeze` replaces it with a pinned commit SHA at install time per ADR-042 §Decision and PRD §R-3 supply-chain hygiene. `pass_filenames: false` (gitleaks scans staged content directly), `stages: [pre-commit]`.
- **New `.aod/scripts/bash/precommit-wrap.sh` wrapper** (NEW): invoked by the pre-commit hook as the entry point for the gitleaks rule. Runs `gitleaks git --pre-commit --redact --staged --verbose --config=.gitleaks.toml` and augments the refused-commit stderr with the four-item structured contract (rule ID + file:line + bypass guidance + docs link to `docs/standards/PRECOMMIT_HOOKS.md`). **FM-5 exit-code-capture pattern**: `rc=$?` is captured BEFORE the augmentation block runs so a failure inside the augmentation cannot mask gitleaks' own exit code. Bash 3.2 compatible (no associative arrays, no `mapfile`, no `&>` redirection). **LOCAL-ONLY scope** — the CI parity workflow does NOT invoke this wrapper because the wrapper writes to stderr instead of SARIF (per spec PM-5 / ADR-042 §Decision).
- **`scripts/init.sh` opt-in prompt block** (DELTA): added an opt-in pre-commit hook prompt at the secret-scanning phase with default `Y` in TTY contexts. Two new flag overrides: `--precommit` forces install regardless of TTY/answer; `--no-precommit` forces skip. Non-TTY contexts (`</dev/null`, CI, scripted invocation) skip the prompt silently. The prompt phase still enforces a pre-commit floor check; if installed but below the floor, init.sh logs a WARN with upgrade guidance and continues. If not installed, init.sh logs a WARN with install guidance from the user's package manager or Homebrew and continues. Init.sh never aborts on pre-commit framework absence or version mismatch — the hook remains opt-in defense-in-depth, not a hard prerequisite.
- **No new adopter-facing env vars**: F-5 does NOT introduce any environment variables beyond the existing `SKIP=gitleaks` framework-level bypass (which has been part of pre-commit for years and is not tachi-specific). The `--precommit` / `--no-precommit` init.sh flags are CLI-level, not env-var-level.
- **Three documented opt-out paths**: (a) `--no-precommit` flag at init.sh time, (b) decline the TTY prompt with `n`, (c) post-init `pre-commit uninstall` from the repo root. Existing adopters who pull F-5 via `make update` / `git pull` do NOT receive an auto-installed hook (FR-010, opt-in semantics) — they must explicitly run `pre-commit install` to enable. The F-5 update is **non-destructive** for existing adopter clones.
- **Bypass mechanisms** (documented in `docs/standards/PRECOMMIT_HOOKS.md` §4): single-commit bypass (`SKIP=gitleaks git commit ...`), inline allow-list comment (`# gitleaks:allow`), `git commit --no-verify` (last-resort, deliberate-bypass — backed by the `gitleaks.yml` CI parity workflow), and `pre-commit uninstall` (full opt-out). The CI parity workflow is the back-stop for the `--no-verify` path: a developer who deliberately bypasses the hook still encounters the gitleaks scan at PR time before merge.
- **No staging/production deployment changes, no secret changes, no monitoring changes, no Docker services, no deployment-environment changes**: F-5 ships zero runtime, daemon, service, or networking surface inside any deployment environment. The hook executes at `git commit` time on the developer's local machine and inside `pull_request`-scoped GitHub Actions jobs. Staging and Production READMEs are intentionally unchanged. `docs/architecture/04_deployment_environments/*.md` are intentionally unchanged.

**Adopter action**:

- **New tachi adopters** (running `scripts/init.sh` for the first time): the prompt at init time is default-Y; pressing `<Enter>` accepts. If `pre-commit` is not installed system-wide, init.sh logs a WARN and continues — install it via your package manager or Homebrew, then run `pre-commit install` from the repo root to enable the hook post-init.
- **Existing tachi adopters** (already running tachi via `make update` / `git pull`): F-5 is opt-in. To enable the hook, install `pre-commit` via your package manager or Homebrew if needed, then run `pre-commit install`. Pulling F-5 does NOT auto-install the hook (FR-010).
- **CI consumers**: the new `gitleaks.yml` workflow runs on `pull_request` events only — pure-`push` workflows are unchanged. SARIF findings appear in GitHub's Security tab → Code Scanning under category `gitleaks`. No CI shell branching changes required (the workflow is independent of the `/aod.deliver` exit-code contract from F139).
- **Maintainers extending the gitleaks ruleset**: edits to `.gitleaks.toml` MUST be paired with a corresponding entry in the per-rule rationale catalog at `docs/standards/PRECOMMIT_HOOKS.md` §3. AC-10 [MANUAL-ONLY] reviewer cross-check at `/aod.deliver` time verifies catalog parity. Bumping gitleaks (e.g., v8.31.x or higher) requires re-testing against `tests/fixtures/gitleaks-rule-interaction/` before merge per ADR-042 §Consequences — and updating the SHA256 checksum in `.github/workflows/gitleaks.yml` from the upstream `gitleaks_<VERSION>_checksums.txt` release artifact. PRD: `docs/product/02_PRD/282-pre-commit-secret-scanning-defaults-2026-05-09.md`. Spec + tasks: `specs/282-pre-commit-secret-scanning-defaults/`. ADR: `docs/architecture/02_ADRs/ADR-042-pre-commit-secret-scanning-default.md`. Policy log + bypass guide: `docs/standards/PRECOMMIT_HOOKS.md`.
