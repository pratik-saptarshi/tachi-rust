...
| E2E | Critical init flow and the CLI analysis-to-artifact flow are explicitly classified by `crates/tachi-core/tests/coverage_audit.rs`; the `E2E-COV*` roadmap expands coverage to the remaining user-facing boundaries. |
| Coverage | `make llvm-cov` is the release-quality local gate. Latest validated run: 84.77% regions / 85.25% lines, with the configured 85% line threshold passing. The current audit has 110 active modules, 94 Rust integration modules, 13 Rust unit modules, 1 Rust smoke module, 2 Rust E2E modules, and 0 support/regression modules; branch coverage is not emitted by the current report. |
