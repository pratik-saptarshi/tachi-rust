...
| E2E | Critical init, CLI analysis-to-artifact, desktop host command, MCP stdio, and composed init/install/update/analysis lifecycle flows are explicitly classified or exercised by `crates/tachi-core/tests/coverage_audit.rs` and the Rust E2E suites; the `E2E-COV*` roadmap retains the cross-boundary failure matrix as the remaining journey. |
| Coverage | `make llvm-cov` is the release-quality local gate. Latest validated run: 84.77% regions / 85.25% lines, with the configured 85% line threshold passing. The current audit has 112 active modules, 94 Rust integration modules, 13 Rust unit modules, 1 Rust smoke module, 4 Rust E2E modules, and 0 support/regression modules; branch coverage is not emitted by the current report. |
